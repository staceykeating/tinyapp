...
app.get("/urls", (req, res) => {
  const  userID  = req.session.user_id;
  let loggedinUser = users[userID];
  let toDisplayURLS = getURLS(userID);

  if (loggedinUser !== undefined) {
    let templateVars = { userID: req.session.user_id, email: req.session.email, urls: toDisplayURLS };
...
app.get("/urls/:shortURL", (req, res) => {
  ...
  if (loggedinUser !== undefined) {
    ...
    if (getURLS(loggedinUser) === '{}') {
      res.send("URL not in user database");
    } else {
